...
perdiff_solo_plot = ggplot(bar_data_perdiff, aes(x = statename_f, y = factor_diff, text = label))+
  ...
  scale_y_continuous(labels = seq(0,50,5), breaks = seq(0,50,5)) +
...
############################################################
# Bar chart of absolute counts (Panel A)
############################################################
bar_data =  bar_data_perdiff %>%
  ...
  mutate(statename_f = fct_reorder(statename, obs_perpop, max)) %>% 
  dplyr::select(statename, statename_f, obs_perpop,  exp_perpop,
                exp_perpop_lb, exp_perpop_ub) %>%
  melt(id.vars = c("statename", "statename_f", "exp_perpop_lb","exp_perpop_ub")) %>%
  mutate(variable = as.character(variable)) %>%
  mutate(variable_f = as.factor(ifelse(variable == "obs_perpop", "Confirmed cases", 
                                       "Estimated infections"))) %>%
  mutate(variable_f = factor(variable_f, levels = c("Confirmed cases", "Estimated infections"))) %>%
  mutate(exp_perpop_lb = ifelse(variable=="obs_perpop", NA, exp_perpop_lb),
         exp_perpop_ub = ifelse(variable=="obs_perpop", NA, exp_perpop_ub))

# create label for interactive plot
bar_labels = bar_data %>% dplyr::select(statename) %>%
  left_join(bar_data %>% filter(variable == "obs_perpop") %>%
  ...
  mutate(label = paste0("<b>", statename, "</b>\n",
                        "Confirmed Cases: ", 
                        format(value.x, big.mark=",", digits=1, scientific=F, trim = TRUE), "\n",
                        "Estimated Cases: ", 
                        format(value.y, big.mark=",", digits=1, scientific=F, trim = TRUE), 
...
bar_data$variable_f = factor(bar_data$variable_f, levels= c("Confirmed cases", "Estimated infections"))
...
nat_bar = nat_bar_solo + ggtitle("A) Cumulative COVID-19 infections per 1,000") 


bar_data_forint = bar_data %>% mutate(variable_f = factor(bar_data$variable_f, 
                                                          levels= c("Estimated infections", "Confirmed cases")))
...
nat_bar_int =  ggplotly(nat_bar_forint + scale_fill_manual("", values = c("#858585","#E5BC4D")) , tooltip="text") %>% 
  ...
  layout(barmode = 'overlay',
         annotations = list(
           text = "A) Cumulative COVID-19 infections per 1,000",
           xref = "paper",
           yref = "paper",
           yanchor = "bottom",
           xanchor = "center",
...
nat_bar_solo_int =  ggplotly(nat_bar_solo_forint + scale_fill_manual("", values = c("#858585","#E5BC4D")) , tooltip="text") %>% 
  ...
  layout(barmode = 'overlay',
         annotations = list(
           text = "Cumulative COVID-19 infections per 1,000",
           xref = "paper",
           yref = "paper",
           yanchor = "bottom",
           xanchor = "center",
...
panel_2 = grid.arrange(perdiff_plot, blank, nrow = 2, heights = c(17, 1.4))

bar_plot = grid.arrange(nat_bar, panel_2, ncol = 2)
